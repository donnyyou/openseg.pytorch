...
class ASP_OC_Module(nn.Module):
    def __init__(self, features, out_features=256, dilations=(12, 24, 36), bn_type=None):
...
class ASP_OC_Module_v2(nn.Module):
    def __init__(self, features, out_features=512, dilations=(12, 24, 36), bn_type=None):
        ...
        self.context = nn.Sequential(BaseOC_Context_Module(in_channels=features, out_channels=out_features,
                                                              key_channels=features//4, value_channels=features//4,
                                                              dropout=0, sizes=([2]), bn_type=bn_type))
        self.conv2 = nn.Sequential(nn.Conv2d(features, out_features, kernel_size=1, padding=0, dilation=1, bias=False),
                                   ModuleHelper.BNReLU(out_features, bn_type=bn_type))
        self.conv3 = nn.Sequential(nn.Conv2d(features, out_features, kernel_size=3, padding=dilations[0], dilation=dilations[0], bias=False),
...
class ASP_OC_Module_v3(nn.Module):
    ...
    def _cat_each(self, feat1, feat2, feat3, feat4, feat5):
        ...
        z = []
        for i in range(len(feat1)):
            z.append(torch.cat((feat1[i], feat2[i], feat3[i], feat4[i]), 1))
        return z
    ...
    def forward(self, x):
        ...
        if isinstance(x, Variable):
            ...
            out = self._cat_each(feat1, feat2, feat3, feat4)
        else:
            raise RuntimeError('unknown input type')
