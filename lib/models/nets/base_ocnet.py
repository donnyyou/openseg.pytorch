...
class BaseOCNet_v2(nn.Module):
    def __init__(self, configer):
        self.inplanes = 128
        super(BaseOCNet_v2, self).__init__()
        self.configer = configer
        self.num_classes = self.configer.get('data', 'num_classes')
        self.backbone = BackboneSelector(configer).get_backbone()
...
class BaseOCNet_Conv3x3(nn.Module):
    def __init__(self, configer):
        self.inplanes = 128
        super(BaseOCNet_Conv3x3, self).__init__()
        self.configer = configer
        self.num_classes = self.configer.get('data', 'num_classes')
        self.backbone = BackboneSelector(configer).get_backbone()
        ...
        from lib.models.modules.base_oc_block import BaseOC_Module_Conv3x3
        self.oc_module = BaseOC_Module_Conv3x3(in_channels=512, out_channels=512,
                                          key_channels=256, value_channels=256,
                                          dropout=0.05, sizes=([1]),
                                          bn_type=self.configer.get('network', 'bn_type'))
        self.cls = nn.Conv2d(512, self.num_classes, kernel_size=1, stride=1, padding=0, bias=True)
