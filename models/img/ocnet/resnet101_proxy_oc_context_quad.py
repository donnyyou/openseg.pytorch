...
class ResNet101ProxyOCContextQuad(nn.Module):
    def __init__(self, configer):
        self.inplanes = 128
        super(ResNet101ProxyOCContextQuad, self).__init__()
    ...
    def forward(self, x):
        x = self.backbone(x)
        context2 = self.context2(x[-3])
        context3 = self.context3(x[-2])
        context4 = self.context4(x[-1])
        contrext_fuse = self.context_fuse(torch.cat([context2, context3, context4], 1))
        res2 = self.cls2(context2)
        res3 = self.cls3(context3)
        res4 = self.cls4(context4)
        res_fuse = self.cls_fuse(contrext_fuse)
        return [res2, res3, res4, res_fuse]
