...
class _ProxyAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # proxy_context = self.f_value(proxy_context.permute(0, 2, 1)).permute(0, 2, 1)
        # proxy_context = F.normalize(proxy_context, p=2, dim=2)
...
class ProxyOC_Module(nn.Module):
    ...
    def forward(self, feats):
        feats_norm =  1 * F.normalize(feats, p=2, dim=1) 
        priors = [stage(feats_norm) for stage in self.stages]
        context = priors[0]
        for i in range(1, len(priors)):
            context += priors[i]
        output = self.conv_bn_dropout(torch.cat([context, feats_norm], 1))
        return output


class ProxyOC_Module_v2(nn.Module):
    """
    Implementation of the ProxyOC module
    Parameters:
        in_features / out_features: the channels of the input / output feature maps.
    ...
    """
    def __init__(self, in_channels, out_channels, key_channels, value_channels, dropout, sizes=([1]), proxy_cnt=128):
        super(ProxyOC_Module_v2, self).__init__()
        self.stages = []
        self.stages = nn.ModuleList([self._make_stage(in_channels, out_channels, key_channels, value_channels, size, proxy_cnt) for size in sizes])        
        self.conv_bn_dropout = nn.Sequential(
            nn.Conv2d(2*in_channels, out_channels, kernel_size=1, padding=0),
    ...
    def forward(self, feats):
        feats_norm =  10 * F.normalize(feats, p=2, dim=1) 
        priors = [stage(feats_norm) for stage in self.stages]
        context = priors[0]
        for i in range(1, len(priors)):
            context += priors[i]
