...
class _SelfAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        inter_map = self.f_query(x)
        key = inter_map.view(batch_size, self.key_channels, -1)
...
class BaseOCS_Module(nn.Module):
    """
    ...
    def __init__(self, in_channels, out_channels, key_channels, value_channels, dropout, sizes=([1])):
        super(BaseOCS_Module, self).__init__()
        self.stages = []
    ...
    def forward(self, feats):
        ...
        context, inter_map = priors[0]
        for i in range(1, len(priors)):
            context += priors[i]
        output = self.conv_bn_dropout(torch.cat([context, feats], 1))
        return output, inter_map


class BaseOCS_Context_Module(nn.Module):
    """
    ...
    def __init__(self, in_channels, out_channels, key_channels, value_channels, dropout=0, sizes=([1])):
        super(BaseOCS_Context_Module, self).__init__()
        self.stages = []
        self.stages = nn.ModuleList([self._make_stage(in_channels, out_channels, key_channels, value_channels, size) for size in sizes])
        self.conv_bn_dropout = nn.Sequential(
