...
CHECKPOINTS_NAME="fs_aspocnetv4_cityscape_ohem_val_seg"$2
...
if [ "$1"x == "train"x ]; then
  ${PYTHON} -u main.py --hypes hypes/cityscape/fs_ocnet_cityscape_seg.json --drop_last y \
                       --phase train --gathered n --loss_balance y --log_to_file n --include_val y \
  ...
  ${PYTHON} -u main.py --hypes hypes/cityscape/fs_ocnet_cityscape_seg.json \
  ...
  cd val/scripts
  ${PYTHON} -u cityscape_evaluator.py --hypes_file ../../hypes/cityscape/fs_ocnet_cityscape_seg.json \
