...
LOG_FILE="./log/cityscape/fs_deeplabv3_cityscape_seg.log"
...
if [ "$1"x == "train"x ]; then
  ...
  ${PYTHON} -u main.py --hypes hypes/cityscape/fs_deeplabv3_cityscape_seg.json \
                       --phase train --gathered n --loss_balance y --log_to_file n \
                       --data_dir /msravcshare/v-ansheng/DataSet/CityScape \
                       --resume_continue y --resume ./checkpoints/cityscape/fs_deeplabv3_cityscape_seg_latest.pth \
                       --pretrained ./pretrained_model/resnet101-imagenet.pth  >> $LOG_FILE 2>&1

elif [ "$1"x == "debug"x ]; then
  ${PYTHON} -u main.py --hypes hypes/cityscape/fs_deeplabv3_cityscape_seg.json --phase debug --gpu 0 > $LOG_FILE 2>&1
